...
Identical plans and contexts produce identical outputs; failures never default to ACCEPT.
"""
...
def validate_graph(plan: CandidatePlan) -> tuple[Diagnostic, ...]:
    ...
    # Iterative DFS avoids coupling accepted plan depth to Python's call stack.
    by_id = {operator.operator_id: operator for operator in plan.operators}
    color: dict[str, int] = {}
    cyclic = False
    for root in ids:
        if color.get(root, 0) != 0 or root not in by_id:
            continue
        stack: list[tuple[str, int]] = [(root, 0)]
        color[root] = 1
        while stack and not cyclic:
            operator_id, next_input = stack[-1]
            inputs = by_id[operator_id].inputs
            if next_input >= len(inputs):
                color[operator_id] = 2
                stack.pop()
                continue
            parent = inputs[next_input]
            stack[-1] = (operator_id, next_input + 1)
    ...
    if plan.output_operator in by_id:
        ...
        reachability_stack = [plan.output_operator]
        while reachability_stack:
            operator_id = reachability_stack.pop()
            if operator_id in reachable or operator_id not in by_id:
                continue
            reachable.add(operator_id)
            reachability_stack.extend(reversed(by_id[operator_id].inputs))
