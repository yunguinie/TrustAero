...
class RelationSchema:
    ...
    @cached_property
    def fields_by_name(self) -> Mapping[str, FieldDescriptor]:
        """Index a propagated schema once instead of rescanning it per reference."""
...
def _topological_order(operators: tuple[Operator, ...]) -> tuple[Operator, ...]:
    ...
    for root in operators:
        if root.operator_id in visited:
            continue
        ...
        while stack:
            ...
            for input_id in reversed(operator.inputs):
                parent = by_id.get(input_id)
                if parent is not None and parent.operator_id not in visited:
