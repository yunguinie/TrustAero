...
def run_multisource_case_study_v2(
    ...
    *,
    ...
    protocol = _load_json(project_root / protocol_path)
    report(1, 9, "verifying four frozen sources and constructing Agent requests")
    artifacts = verify_multisource_case_artifacts(
        project_root, Path(str(protocol["prepared_manifest_path"]))
    )
    for override in protocol.get("prepared_artifact_overrides", []):
        artifact_id = str(override["artifact_id"])
        replacement = next(
            (item for item in artifacts["outputs"] if str(item["artifact_id"]) == artifact_id),
            None,
        )
        if replacement is None:
            raise MultisourceCaseStudyError(f"Unknown prepared artifact override: {artifact_id}")
        path = project_root / "data" / str(override["relative_path"])
        digest = sha256_file(path)
        if path.stat().st_size != int(override["byte_size"]) or digest != str(override["sha256"]):
            raise MultisourceCaseStudyError(
                f"Refreshed snapshot does not match its frozen override: {artifact_id}"
            )
        replacement.update(
            {
                "relative_path": str(override["relative_path"]),
                "row_count": int(override["row_count"]),
    ...
    catalog_payload = _load_json(project_root / str(protocol["catalog_path"]))
    for dataset_id, snapshot in protocol.get("catalog_snapshot_overrides", {}).items():
        dataset = next(
            item for item in catalog_payload["datasets"] if item["dataset_id"] == dataset_id
        )
        dataset["versions"] = sorted(set(dataset["versions"]) | {snapshot})
        dataset["default_version"] = snapshot
    ...
    try:
        ...
        required_result_digest = protocol.get("required_result_digest")
        if (
            first.row_count <= 0
            or first.row_count != second.row_count
            or first.result_digest != second.result_digest
            or (
                required_result_digest is not None and first.result_digest != required_result_digest
            )
        ):
    ...
    run_dir = project_root / results_path / run_id
    summary: dict[str, Any] = {
        "schema_version": 1,
        "status": protocol.get("result_status", "PASS_MULTISOURCE_CASE_STUDY_V2_COMPLETE_LOOP"),
        ...
        "protocol_sha256": sha256_file(project_root / protocol_path),
    ...
    _atomic_json(project_root / results_path / "latest_run.json", {"run_id": run_id})
    return run_dir
