"""Run and analyze a frozen governed TPC-H Q6 timing protocol."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("experiments/configs/tpch_q6_formal_v1.json"),
    )
    ...
    config = load_tpch_q6_formal_config(root / args.config)
    run_dir = run_tpch_q6_formal(
        config,
        project_root=root,
        show_progress=args.progress,
        resume_run_id=args.resume_run_id,
    )
    acceptance = analyze_tpch_q6_formal(run_dir)
