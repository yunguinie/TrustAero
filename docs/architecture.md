...
| Logical validation | `ValidatedLogicalPlan` | operator graph, catalog references, typed expressions, policy decisions, inferred obligations, safe rewrites | logical approval alone establishes physical or execution correctness |
...
- the Validator;
- the versioned catalog and policy store;
- the Candidate Generator and Planner;
- the Compiler and controlled DuckDB execution path;
- the Certificate Builder and its execution-event inputs.
...
`ApprovedPhysicalPlan` is a pre-execution specification rather than arbitrary
SQL. It freezes the selected physical operator skeleton. For the supported
fragment, the current Compiler lowers that skeleton to the controlled DuckDB
execution path; unsupported operators fail before execution.
